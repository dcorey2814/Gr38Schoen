# Gr38Schoen
Code used in the paper <a href="https://arxiv.org/abs/2206.14993"> The Grassmannian of 3-planes in C^8</a> is sch\"on by Daniel Corey and Dante Luber.
...
## The seconday fan structure $\operatorname{TGr}^{\circ}(3,8)$. 
...
```group38``` is a polymake data file containing an ```Array<Array<Int>>```. Denote by $\mathfrak{S}_{n}$ the symmetric group on $[n]$.  This file records the subgroup of $\mathfrak{S}_{56}$  isomorphic to  $\mathfrak{S}_8$ induced by the action of $\mathfrak{S}_8$ on $\binom{[8]}{3}$ given by

$\sigma \cdot (i,j,k ) = (\sigma(i),\sigma(j),\sigma(k))$

Each of these permutation in $\mathfrak{S}_{56}$ is recorded in the standard ``one-line'' notation of a permutation, i.e., $\sigma = (\sigma(1), \sigma(2), \ldots, \sigma(56))$.
